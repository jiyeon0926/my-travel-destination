...
### Redisson 분산 락을 사용하여 동시성 제어를 한 이유
동시성 제어 방법으로는 비관적 락, 낙관적 락, Redis를 활용한 분산 락 방법이 있습니다. </br>
비관적 락 방법은 현재 트랜잭션이 접근한 데이터에 행 단위로 락을 걸어 다른 트랜잭션이 읽기/쓰기 접근을 하지 못하게 하기 때문에 데이터의 일관성을 보장할 수 있습니다. </br>
하지만, 동시 접속자가 많을 경우에는 락 대기 시간으로 인해 성능 저하가 생길 수 있습니다. </br>
낙관적 락 방법은 버전 컬럼을 추가하여 변경 사항이 생겼을 경우에만 버전이 올라가고, 버전이 다르면 업데이트가 불가능 합니다. </br>
현재 프로젝트에서는 재고 차감 및 복구 과정으로 인해 데이터 변경이 자주 일어나서 버전 충돌이 발생할 수 있습니다. </br> 
충돌을 해결하기 위한 복구 및 재시도 과정에 대한 시간이 필요합니다. </br> </br>
분산 락은 락을 획득한 프로세스나 스레드만 공유 자원이나 임계 구역에 접근할 수 있도록 하는 방법입니다. </br>
Redis는 메모리에 데이터를 저장하므로 조회 및 처리 속도가 관계형 데이터베이스보다 빨라서 Redis를 활용해 분산 락을 구현할 수 있습니다. </br>
Lettuce 라이브러리를 사용한 분산 락은 락에 대한 타임아웃이 없어서 락을 해제하지 못할 경우, 데드락을 유발할 수 있습니다. </br>
반면, Redisson 분산 락은 락이 유지되는 시간을 설정할 수 있기 때문에 자동으로 락의 소유권을 회수해 데드락을 방지할 수 있습니다. </br> </br>

결과적으로 성능과 확장성을 고려하여 Redisson 분산 락을 선택하였습니다. </br>
