...
## 📍 핵심 기능
- 블로그
  - 제목, 여행 기간, 예상 경비, 총 경비, 내용 작성
  - 파일 크기 5MB 이내로 이미지 업로드 가능
...
## 🔒 로그인 및 로그아웃
이 프로젝트는 인증 기능 자체가 핵심이 아니므로, 인증 구조를 최소한으로 설계하였습니다. </br>
Refresh Token 기반의 토큰 재발급 흐름은 적용하지 않았으며, Access Token만 발급하는 방식으로 구현하였습니다. </br>
로그아웃 시에는 사용 중이던 Access Token을 Redis에 저장하여 블랙리스트로 관리합니다. </br>
